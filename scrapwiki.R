...
sortLeJson<-function(date,idversionchoisie){
  ...
  textesansbackslach<-gsub(":\\[\\[Port of Berdiansk\\]\\]",":'[[Port of Berdiansk]]'",textesansbackslach)
...
textesansbackslach<-gsub(' label = \\[\\[169th Training Centre \\(Ukraine\\)\\]\\],' , ' label = "[[169th Training Centre (Ukraine)]]",',textesansbackslach)
  ...
  textesansbackslach<-gsub('-- Kramatorsk Raion','',textesansbackslach)
textesansbackslach<-gsub('position=','"position":',textesansbackslach)
